
#Define the path to the remote hosts file
$remoteHostsFile = "C:\SDPU\Test\Test-Remote-Host.txt"

#Define the log file path (unique based on SV identifier)
$logFile = "C:\SDPU\Test\log file\SV-253393_LogFile.txt"

#Define the registry path and expected values
$registryPath = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection"
$registryValueName = "AllowTelemetry"
$allowedValues = @(0, 1) # Security (0) or Basic (1)

Clear-Host

#Read the remote hosts file
    if (-Not (Test-Path $remoteHostsFile)) {
    <# ERROR: Hosts file not found #>
    "ERROR: Hosts file not found at $remoteHostsFile." | Out-File -FilePath $logFile
    exit
    }

$remotehosts = Get-Content -Path $remoteHostsFile

foreach ($remotehost in $remotehosts) {
    
    Write-Host "Processing remote host: $remotehost" -ForegroundColor Yellow
    #"$remotehost, Processing" | Out-File -FilePath $logFile -Append

    ## Check if the host is online
    if (-Not (Test-Connection -ComputerName $remotehost -Count 1 -Quiet)) {
        
        Write-Host "WARNING: Remote host $remotehost is offline. Skipping." -ForegroundColor Yellow
        "$remotehost, Offline" | Out-File -FilePath $logFile -Append
        continue
    }


## Execute commands on the remote host
try {
    $result = Invoke-Command -ComputerName $remotehost -ScriptBlock {
        param ($registryPath, $registryValueName, $allowedValues)


        ## Check if the registry path exists
        if (-Not (Test-Path $registryPath)) {
            New-Item -Path $registryPath -Force | Out-Null
        }


        ## Get the current value of the registry key
        $currentValue = (Get-ItemProperty -Path $registryPath -Name $registryValueName -ErrorAction SilentlyContinue).$registryValueName


        ## Set the value if it is not allowed or missing
        if ($currentValue -notin $allowedValues) {
            Set-ItemProperty -Path $registryPath -Name $registryValueName -Value 0
            "0"
        } else {
            $currentValue
        }
    } -ArgumentList $registryPath, $registryValueName, $allowedValues -ErrorAction Stop


    "$remotehost, $result - Security" | Out-File -FilePath $logFile -Append
    if ($result -eq 0) {
        <# SUCCESS: Telemetry level corrected #>
        Write-Host "SUCCESS: Telemetry level corrected to Security (0) on $remotehost." -ForegroundColor Green
    } else {
        <# SUCCESS: Telemetry level compliant #>
        Write-Host "SUCCESS: Telemetry level already compliant on $remotehost." -ForegroundColor Green
    }
} catch {
    <# ERROR: Failed to process #>
    "$remotehost, Error" | Out-File -FilePath $logFile -Append
    Write-Host "ERROR: Failed to process $remotehost. $_" -ForegroundColor Red
}
}
Write-Host "Script execution completed." -ForegroundColor Green